...
class TmpBufferManager final {
 public:
  TmpBufferManager(int32_t capacity, void* ptr, const int32_t box_num,
                   const int32_t anchor_boxes_elem_cnt)
      : capacity_{capacity},
        select_inds_elem_cnt_{box_num},
        anchor_boxes_tmp_elem_cnt_{anchor_boxes_elem_cnt} {
    const int32_t select_inds_aligned_bytes =
        GetCudaAlignedSize(select_inds_elem_cnt_ * sizeof(int32_t));
    ...
    temp_storage_ptr_ = reinterpret_cast<void*>(reinterpret_cast<char*>(anchor_boxes_tmp_ptr_)
...
__global__ void SetOutBoxes(const T* bbox_ptr, const int32_t* origin_image_info_ptr,
  ...
  CUDA_1D_KERNEL_LOOP(i, index_num) {
    ...
    float box_w = std::exp(bbox_ptr[box_index * 4 + 2]) * anchor_boxes_ptr[2 * ibox] / image_width;
    float box_h =
 ...
 private:
  void Compute(user_op::KernelComputeContext* ctx) const override {
    ...
    FOR_RANGE(int32_t, im_index, 0, bbox->shape().At(0)) {
      ...
      T* out_bbox_ptr = out_bbox->mut_dptr<T>() + im_index * out_bbox->shape().Count(1);
      T* out_probs_ptr = out_probs->mut_dptr<T>() + im_index * out_probs->shape().Count(1);
      ...
      CudaCheck(SelectOutIndexes(ctx->device_ctx()->cuda_stream(), probs_ptr,
                                 buf_manager.TempStoragePtr(), buf_manager.SelectIndsPtr(),
                                 valid_num_ptr, buf_manager.TempStorageBytes(), box_num, probs_num,
                                 prob_thresh));
      SetOutProbs<<<BlocksNum4ThreadsNum(box_num * probs_num), kCudaThreadsNumPerBlock, 0,
                    ctx->device_ctx()->cuda_stream()>>>(probs_num, probs_ptr,
                                                        buf_manager.SelectIndsPtr(), valid_num_ptr,
                                                        out_probs_ptr, prob_thresh, max_out_boxes);
      SetOutBoxes<<<BlocksNum4ThreadsNum(box_num), kCudaThreadsNumPerBlock, 0,
                    ctx->device_ctx()->cuda_stream()>>>(
          bbox_ptr,
          origin_image_info->dptr<int32_t>() + im_index * origin_image_info->shape().Count(1),
          buf_manager.SelectIndsPtr(), valid_num_ptr, buf_manager.AnchorBoxesTmpPtr(), out_bbox_ptr,
          ctx->Attr<int32_t>("layer_height"), ctx->Attr<int32_t>("layer_width"), layer_nbox,
...
#define REGISTER_YOLO_DETECT_GPU_KERNEL(dtype)                                                   \
  REGISTER_USER_KERNEL("yolo_detect")                                                            \
      ...
      .SetInferTmpSizeFn([](user_op::InferContext* ctx) {                                        \
        ...
        /* anchor_boxes */                                                                       \
        const int32_t anchor_boxes_tmp_aligned_bytes =                                           \
            GetCudaAlignedSize(2 * layer_nbox * sizeof(int32_t));                                \
        /* CUB Temp Storage */                                                                   \
        int32_t temp_storage_bytes =                                                             \
            InferTempStorageForCUBYoloDetect(box_num, probs_num, prob_thresh);                   \
