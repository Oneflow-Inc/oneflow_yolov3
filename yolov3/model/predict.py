...
import oneflow_detection
from oneflow_detection.ops.yolo_detect import yolo_detect

parser = argparse.ArgumentParser(description="flags for multi-node and resource")
parser.add_argument("-g", "--gpu_num_per_node", type=int, default=1, required=False)
parser.add_argument("-t", "--train_dir", type=str, required=False)
parser.add_argument("-load", "--model_load_dir", type=str, default="yolo/of_model/yolov3_model_python/", required=False)
parser.add_argument(
...
flow.config.load_library(oneflow_detection.lib_path())
...
if __name__ == "__main__":
    ...
    def create_callback(step):
        ...
        def callback(ret):
            ...
            print(yolo_pos.shape, yolo_prob.shape)
            if step == 0:
